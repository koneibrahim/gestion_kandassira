...
$facture=pg_fetch_assoc($pfacture);*/
 ...
 echo'<tr>
  ...
  echo'</thead>
   ...
   echo '<tr class="">';
		echo '<td colspan="">Facture N°:<b class="rouge" style="color:red"> '.$id_fa.'</b></td>';
	echo '</tr>';
	echo '<tr class="">';
  ...
  while($ligne=pg_fetch_assoc($dfacture)){
	echo'<tr>';
		...
		echo'<td style="width: 30%; text-align: left; border: solid 1px black>'.$ligne['type_la'].'</td>';
		echo'<td style="width: 30%; text-align: center; border: solid 1px black>'.number_format($ligne['prix_uni'],0,' ',' ').'<sup>FCFA</sup></td>';
